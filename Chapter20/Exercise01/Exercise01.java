...
public class Exercise01 {
    public static void main(String[] args) {
        ...
        while (theHuman.hasNext()) {
            ...
            human.getName().ifPresent((String s) -> System.out.print(s + "\t"));

            // the age is optional, if not present, insert a tab
            human.getAge().ifPresent((Integer s) -> System.out.print(s + "\t"));

            if(human.getAge().isEmpty()) {
                System.out.print("\t");
            }

            // the gender is optional, but since there are no other
            // parameters, we do not need to add an extra tabulator
            human.getGender().ifPresent((String s) -> System.out.print(s));
